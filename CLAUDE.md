...
primary editor. Read `HANDOFF.md` first — it holds the plan, the build order and
the reasoning. This file holds the working rules.
...
4. **Respect the layer order** in `HANDOFF.md`. Don't add R tooling while the
   completion layer is still unsettled. If asked for something out of order, say
   so and confirm before proceeding.
